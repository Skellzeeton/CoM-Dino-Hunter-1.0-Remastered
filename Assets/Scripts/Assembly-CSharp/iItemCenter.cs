...
public class iItemCenter
{
	...
	public bool Load()
	{
		...
		{
			if (!Utils.FileGetString("item.xml", ref content))
			{
				return false;
			}
		}
		...
		{
			TextAsset textAsset = (TextAsset)Resources.Load(PrefabManager.GetPath(3009), typeof(TextAsset));
			if (textAsset == null)
			{
				return false;
			}
