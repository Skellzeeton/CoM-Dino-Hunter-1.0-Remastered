	...
	public bool Load()
	{
		...
		foreach (XmlNode childNode in documentElement.ChildNodes)
		{
			...
			if (MyUtils.GetAttribute(childNode, "attackmodevalue", ref value))
			{
				...
				{
					cWeaponInfoLevel.ltAttackModeValue.Add(ServerX.ParseFloat(array[i]));
				}
			}
			...
			{
				cWeaponInfoLevel.fDamage = ServerX.ParseFloat(value);
			}
			if (MyUtils.GetAttribute(childNode, "critical", ref value))
			{
				cWeaponInfoLevel.fCritical = ServerX.ParseFloat(value);
			}
			if (MyUtils.GetAttribute(childNode, "criticaldmg", ref value))
			{
				cWeaponInfoLevel.fCriticalDmg = ServerX.ParseFloat(value);
			}
			if (MyUtils.GetAttribute(childNode, "shootspeed", ref value))
			{
				cWeaponInfoLevel.fShootSpeed = ServerX.ParseFloat(value);
			}
			if (MyUtils.GetAttribute(childNode, "msdownshoot", ref value))
			{
				cWeaponInfoLevel.fMSDownRateShoot = ServerX.ParseFloat(value);
			}
			if (MyUtils.GetAttribute(childNode, "msdownequip", ref value))
			{
				cWeaponInfoLevel.fMSDownRateEquip = ServerX.ParseFloat(value);
			}
			if (MyUtils.GetAttribute(childNode, "precise", ref value))
			{
				cWeaponInfoLevel.fPrecise = ServerX.ParseFloat(value);
			}
			...
			{
				cWeaponInfoLevel.fElementUp = ServerX.ParseFloat(value);
			}
			...
			{
				cWeaponInfoLevel.fElementDown = ServerX.ParseFloat(value);
			}
