...
public class CMobInfoLevel
{
    ...
    public int GetDropItemCount()
    {
        ...
        {
            array[i] = (i == 0) ? arrDropCountRate[i] : array[i - 1] + (float)arrDropCountRate[i];
        }
        float num = Random.Range(0f, array[arrDropCountRate.Length - 1]);
        for (int j = 0; j < arrDropCount.Length; j++)
        {
            if (num <= array[j]) return arrDropCount[j];
        }
        return -1;
    }
