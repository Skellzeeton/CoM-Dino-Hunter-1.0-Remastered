...
public class iSpawnBullet_Parabola : iSpawnBullet
{
	...
	protected Vector3 m_v3VelocityCur;
	...
	public override void Initialize(int nUID, int nID, Vector3 v3Pos, Vector3 v3Force, int[] arrFunc, int[] arrValueX, int[] arrValueY)
	{
		m_nOwnerUID = nUID;
		m_v3Src = v3Pos;
		m_v3VelocityBase = v3Force;
		m_v3VelocityCur = m_v3VelocityBase;
		m_bFalling = v3Force.y <= 0f || fGravity == 0f;
		for (int i = 0; i < 3; i++)
		{
