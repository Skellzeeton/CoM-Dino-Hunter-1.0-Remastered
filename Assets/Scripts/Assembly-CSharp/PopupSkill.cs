...
public class PopupSkill : MonoBehaviour
{
	...
	public void SetInfo(ScrollList_SkillItem m_item)
	{
		...
		Vector3 position = new Vector3(label_skill_name.transform.localPosition.x + x - 4f, label_skill_name.transform.localPosition.y, label_skill_name.transform.localPosition.z);
	...
	public void CheckScrollChoose()
	{
		...
		global::EventCenter.EventCenter.Instance.Publish(this, new TUIEvent.SendEvent_SceneSkill("TUIEvent_SkillChoose", wparam, skillID));
	}
	...
	public void SkillUpdate()
	{
		if (!item_choose.ReachLevelMax())
		{
			...
			case UnitType.Gold:
				num = top_bar.GetGoldValue();
				num -= price;
				Debug.Log("cost:" + price + unit_type.ToString());
				if (num >= 0)
				{
					top_bar.SetGoldValue(num);
